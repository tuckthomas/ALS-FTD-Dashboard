...
def get_source_priority(feed_url, entry=None):
    ...
    # 1. Scientific / Highly Specialized
...
def fetch_and_process_news():
    ...
    # Key: norm_title -> Value: {priority, entry_data, feed_url}
    ...
    # Load recent DB articles for cross-run duplicate prevention
    ...
    db_normalized_titles = {normalize_title(a.title) for a in recent_articles if a.title}
    ...
    for feed_url in all_feeds:
        ...
        try:
            ...
            for entry in feed.entries:
                ...
                if not norm_title or norm_title in db_normalized_titles:
                    continue
                
                # If we have this story in the buffer, resolve conflict
                if norm_title in article_buffer:
                    existing = article_buffer[norm_title]
                    
                    # 1. Check if one is significantly older (> 4 hours difference)
                    ...
                    if time_diff_seconds > 14400: # New one is > 4 hours older
                        ...
                        # 2. If dates are close, prefer higher quality source
                        if priority < existing['priority']:
                            replace = True
                        elif priority > existing['priority']:
                            replace = False
                        else:
                            # 3. Absolute oldest wins if priority is same
                ...
                if matched_keywords:
                    ...
                    article_buffer[norm_title] = {
                        ...
                        'data': {
                            ...
                            'image_url': None # Extracted below
    ...
    # 3. Save buffered articles to DB
